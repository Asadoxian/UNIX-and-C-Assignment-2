...
{
	char *filename, *filepath, *fileperms, *strNewFile;
	filename = malloc(sizeof(*filename));
	filepath = malloc(sizeof(*filepath));
	fileperms = malloc(sizeof(*fileperms));
	strNewFile = malloc(sizeof(*strNewFile));
...
{
	int i = 0;
	char *buffer, *label, *value, *strOutPut;
	buffer = malloc(sizeof(*buffer));
	label = malloc(sizeof(*label));
	value = malloc(sizeof(*value));
	strOutPut = malloc(sizeof(strOutPut));
...
void view_file(char *CurrentUser)
{
	char *FileName, *strOutPut;
	strOutPut = malloc(sizeof(*strOutPut));
	FileName = malloc(sizeof(*FileName));
	CurrentUser = malloc(sizeof(*CurrentUser));
	int FilePermit = 0;
...
void delete_file(char CurrentUser[30])
{
	int FilePermit = 0;
	int i = 0;
	char *buffer, *FileName, *FullPath, *FilePath, *label, *value, *LineOut, *OldName, *NewName;
	...
	label = malloc(sizeof(*label));
	value = malloc(sizeof(*value));
	LineOut = malloc(sizeof(*LineOut));
	OldName = malloc(sizeof(*OldName));
...
void create_listen_socket()
{
	...
	char *NewFriend;
	char sockbuff[256];
	NewFriend = malloc(sizeof(*NewFriend));

	struct sockaddr_in appaddr, cli_addr;
	...
	appaddr.sin_addr.s_addr = INADDR_ANY;
	appaddr.sin_port = htons(PortNo);

	if (bind(sockcl, (struct sockaddr *) &appaddr, sizeof(appaddr)) < 0)
	{
	...
	sockfr = accept(sockcl, (struct sockaddr *) &cli_addr, &clilen);
	if (sockfr < 0)
	{
	...
	fraccept = read(sockfr, sockbuff, 255);
